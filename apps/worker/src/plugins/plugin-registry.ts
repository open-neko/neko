...
export class PluginRegistry {
  ...
  authSignInReady(): boolean {
    const gaps = this.getAuthEnvGaps();
    return gaps !== null && gaps.length === 0;
  }

  /**
   * Names of required env vars the auth plugin still needs before its
   * sign-in flow can go live (empty array = env complete). Null when no
   * auth plugin is installed. Key names only — never values.
   */
  getAuthEnvGaps(): string[] | null {
    ...
    for (const req of entry.permissions.env ?? []) {
      ...
      if (typeof value !== "string" || value.length === 0) gaps.push(req.key);
    }
    return gaps;
  }

  /**
   * Declared env keys of the auth plugin (required or optional) that
   * currently hold a value. Names only — lets the setup UI show which
   * email provider is configured without touching any secret value.
   */
  getAuthConfiguredEnvKeys(): string[] {
    ...
    if (!entry) return [];
    const env = mergeEnv(entry, this.secrets);
    return (entry.permissions.env ?? [])
      .map((req) => req.key)
      .filter((key) => typeof env[key] === "string" && env[key].length > 0);
  }

  /** Declared env keys for the auth plugin — the allowlist for setAuthSecrets. */
  getAuthDeclaredEnvKeys(): string[] {
    const provider = this.getAuthProvider();
    if (!provider) return [];
    const entry = this.state.entriesByPluginId.get(provider.pluginId);
    if (!entry) return [];
    return (entry.permissions.env ?? []).map((req) => req.key);
  }
   ...
   */
  /**
   * Mint any missing `autogenerate` env values (48 random bytes,
   * base64url) and persist them to the secrets file. Covers plugins
   * installed by an older CLI that prompted instead of generating, and
   * keeps `authSignInReady` from staying false over a secret no human
   * was ever supposed to type.
   */
  private async backfillAutogeneratedSecrets(state: ManifestState): Promise<void> {
    for (const [, entry] of state.entriesByPluginId) {
      const merged = mergeEnv(entry, this.secrets);
      for (const req of entry.permissions.env ?? []) {
        if (!req.autogenerate) continue;
        const value = merged[req.key];
        if (typeof value === "string" && value.length > 0) continue;
        try {
          await this.setPluginSecret(
            entry.name,
            req.key,
            randomBytes(48).toString("base64url"),
          );
          console.log(
            `[plugin-registry] generated ${req.key} for ${entry.name}`,
          );
        } catch (err) {
          console.warn(
            `[plugin-registry] could not generate ${req.key} for ${entry.name}: ${err instanceof Error ? err.message : err}`,
          );
        }
      }
    }
  ...
  async deletePluginSecret(pluginName: string, key: string): Promise<void> {
    const before: FullSecretsFile = { env: this.secrets, operators: this.operators };
    const { store: nextEnv, removed } = unsetSecret(before.env, pluginName, key);
    if (!removed) return;
    this.secrets = nextEnv;
    await writeFullSecretsFile({ env: nextEnv, operators: before.operators }, this.options.secretsConfigDir);
    this.scrubber = createScrubber(
