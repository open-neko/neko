...
export async function ensureGraphjinGuard(
  ...
  const script = [
    ...
    "  token=$(node - <<'NODE' 2>/dev/null || true",
    "const fs = require('node:fs');",
    "const path = require('node:path');",
    "const roots = [process.env.XDG_CONFIG_HOME, process.env.HOME].filter(Boolean);",
    "const suffixes = [",
    "  ['graphjin', 'client.json'],",
    "  ['.config', 'graphjin', 'client.json'],",
    "  ['Library', 'Application Support', 'graphjin', 'client.json'],",
    "];",
    "for (const root of roots) {",
    "  for (const suffix of suffixes) {",
    "    try {",
    "      const cfg = JSON.parse(fs.readFileSync(path.join(root, ...suffix), 'utf8'));",
    "      if (typeof cfg.token === 'string' && cfg.token.trim()) {",
    "        process.stdout.write(cfg.token.trim());",
    "        process.exit(0);",
    "      }",
    "    } catch {}",
    "  }",
    ...
    "status=0",
    "if [[ -n \"$token\" ]]; then",
    `  out=$("${graphjinBinary}" "cli" --header "Authorization: Bearer $token" "\${@:2}") || status=$?`,
    "else",
    `  out=$("${graphjinBinary}" "cli" "\${@:2}") || status=$?`,
    "fi",
