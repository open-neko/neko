...
| 3.1.6 | Non-privileged accounts for non-security functions | | L2 | L3 | Product | Partial | Two roles exist (`admin`/`member`, `apps/web/src/lib/auth.ts` `defaultRoleForGroups`); day-to-day use needs no admin role. Finer-grained roles are not yet available; `PLUGINS.md` notes admin/member separation for install policy is future work. |
...
| 3.3.4 | Alert on audit logging failure | | L2 | L3 | Product | Gap | Chain appends are best-effort and an append failure only emits a process warning. Because a failed append creates no sequence number, `verifyAuditChain` cannot detect that missing event (`packages/llm/src/workflows/audit-chain.ts`). No real-time logging-failure alert exists. |
...
| 3.5.6 | Disable identifiers after inactivity | | L2 | L3 | Product | Partial | Admin-driven disable exists and takes effect immediately — a deactivated user's cookie is dead, not just their sign-in (`db/migrations/0035_app_user_disabled.sql`; `getCurrentUser` filters `disabled_at`). No *automatic* inactivity-based disable. |
...
1. **3.1.6 — Partial:** admin/member roles exist, but finer-grained non-privileged roles are not available.
2. **3.3.4 — Gap:** a failed audit-chain append emits only a process warning; it creates no detectable chain entry and no real-time logging-failure alert.
3. **3.5.6 — Partial:** administrators can disable identifiers immediately, but OpenNeko does not automatically disable them after an organization-defined inactivity period.
4. **3.13.11 — Gap:** OpenNeko's shipped Node/OpenSSL and Go cryptographic modules are not FIPS 140-validated builds. This matters wherever those modules provide CUI confidentiality rather than a client-controlled FIPS boundary doing so.
5. **CM 3.4.2e — Partial:** product status and policy reconciliation exist, but continuous automated drift detection/remediation does not cover every OpenNeko component.
6. **IA 3.5.1e — Partial:** OpenShell gateway links use mTLS and per-sandbox JWTs, but bidirectional cryptographic authentication is not universal across every internal component hop.
